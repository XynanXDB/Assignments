...
#define oceanWidth 50
#define oceanLength 20

using namespace std;

const int oceanVertices = (oceanWidth - 1) * (oceanLength - 1) * 6;
const int oceanArraySize = oceanVertices * 3;
const int oceanUV = oceanVertices * 2;
...
private:
	...
	GLfloat mRectVertices[oceanArraySize];
	GLfloat mRectUV[oceanUV];
	...
	void Ocean(const Matrix& viewMatrix, GLfloat magnitude) {
		...
		for (int i = 0; i < oceanWidth; i++) {
			for (int j = 0; j < oceanLength; j++) {
				...
				float yFirst = sin(i + time);
				float yThen = sin(i + 1 + time);

				mRectVertices[count] = -0.5 + (i * 2);
				count++;
				mRectVertices[count] = yFirst;
				count++;
				mRectVertices[count] = -0.5 + (j * 2);
				count++;


				mRectVertices[count] = -0.5 + (i * 2);
				count++;
				mRectVertices[count] = yThen;
				count++;
				mRectVertices[count] = 0.5 + (j * 2);
				count++;
		...
		glDrawArrays(GL_TRIANGLES, 0, oceanVertices);
		//x Deactivate vertex array after drawing
		glDisableClientState(GL_VERTEX_ARRAY);
